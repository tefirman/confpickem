...
class ConfidencePickEmSimulator:
    ...
    def simulate_picks(
        self, fixed_picks: Dict[str, Dict[str, int]] = None, player_data: pd.DataFrame = None
    ) -> pd.DataFrame:
        ...
        for player_name, picks in fixed_picks.items():
            ...
            for points in picks.values():
                if not isinstance(points, int) or points < 1 or points > num_games:
                    raise ValueError(
                        f"Invalid confidence points: {points}. Must be between 1 and {num_games}"
                    )
        ...
        for sim in range(self.num_sims):
            ...
            base_probs = vegas_probs.reshape(1, -1) * (
                1 - crowd_following.reshape(-1, 1)
            ...
            for p, player in enumerate(self.players):
                ...
                chosen_conf = np.where(player_picks, crowd_home_conf, crowd_away_conf)
                opposing_conf = np.where(player_picks, crowd_away_conf, crowd_home_conf)
                ...
                # Blend signals
                blended_conf = chosen_conf * (1 + conf_diff) * conf_following[p] + vegas_conf * (
                    1 - conf_following[p]
                )
                ...
                incomplete_game_indices = [
                    i for i, game in enumerate(self.games) if game.actual_outcome is None
                ]

                if len(incomplete_game_indices) > 0 and len(available_conf) >= len(
                    incomplete_game_indices
                ):
                    ...
                    score_index_pairs = list(
                        zip(-incomplete_conf_scores, incomplete_game_indices)
                    )  # Negative for descending
                    score_index_pairs.sort()  # Sort by confidence (desc), then by game index (asc) for ties

                    available_conf_list = sorted(
                        list(available_conf), reverse=True
                    )  # Highest first
                    ...
                    if not player_row.empty:
                        ...
                        for g_idx, game in enumerate(self.games):
                            if game.actual_outcome is not None:  # Completed game
                                ...
                                if (
                                    pick_col in player_data.columns
                                    and conf_col in player_data.columns
                                ):
    ...
    def analyze_results(self, picks_df: pd.DataFrame, outcomes: np.ndarray) -> Dict:
        ...
        picks_df["correct"] = picks_df.apply(
            lambda x: outcomes[
                x.simulation, [g.away_team + "@" + g.home_team for g in self.games].index(x.game)
            ]
            == x.picked_home,
            axis=1,
        )
        ...
        win_pct_series = pd.Series(
            [x["win_pct"] for x in win_pcts], index=[x["player"] for x in win_pcts]
        )
    ...
    def optimize_picks(
        ...
        remaining_games = [
            g
            for g in sorted(self.games, key=lambda g: abs(g.vegas_win_prob - 0.5), reverse=True)
        ...
        for game in remaining_games:
            ...
            print(
                f"  Chose {best_pick} with {best_points} points for win probability {best_win_prob:.4f}"
            )

        return optimal

    def _player_game_pick(
        self, player_row: pd.Series, game_idx: int, game: Game
    ...
    def _build_analytic_field(
        ...
        locked_pending = [
            (not completed[i]) and (g.picks_locked or _kicked_off(g)) for i, g in enumerate(games)
        ]
        ...
        opponents = [
            (p.crowd_following, p.confidence_following)
        ...
        opp_types = _an.build_opponent_types(
            ...
            crowd_away_conf,
            opponents,
            completed_picks=opp_completed,
            actual_outcomes=actual_outcomes,
            locked_picks=opp_locked,
            max_types=max_opponent_types if any(frozen) else None,
        )
        rng = np.random.default_rng(seed)
        outcomes = _an.sample_outcomes(vegas_home, n_outcomes, rng, actual_outcomes=actual_outcomes)
    ...
    def optimize_picks_analytic(
        ...
        * each opponent's *actual* frozen picks are folded into the field model
        ...
        if len(locked_vals) != len(set(locked_vals)):
            raise ValueError(
                "Locked picks (frozen games + fixed picks) reuse " "a confidence value"
            )
        ...
        if verbose:
            ...
            print(f"analytical P(win): chalk {pwin(cph, cpts):.4f} " f"-> optimized {val:.4f}")

        return {
            (games[i].home_team if ph[i] else games[i].away_team): int(pts[i]) for i in range(n)
        }

    def assess_game_importance(
        self,
        player_name: str,
        ...
        if picks_df is not None:
            ...
            for i, game in enumerate(games):
                game_id = f"{game.away_team}@{game.home_team}"
                sel = picks_df[(picks_df.player == player_name) & (picks_df.game == game_id)]
        ...
        if not got_all:
            ...
            if not points.all():  # slate still incomplete -> simulate one
                ...
                for i, game in enumerate(games):
                    ...
                    game_id = f"{game.away_team}@{game.home_team}"
                    row = sim_df[
                        (sim_df.player == player_name)
                        & (sim_df.game == game_id)
                        & (sim_df.simulation == s0)
                    ].iloc[0]
                    pick_home[i] = bool(row.picked_home)
                    points[i] = int(row.confidence)

        p_home_wins, p_away_wins, base = _an.game_importance(pwin, outcomes, pick_home, points)
    ...
    def compare_slates(
        ...
        for label, picks in slates.items():
            ...
            missing = [
                f"{games[i].away_team}@{games[i].home_team}"
                for i in range(n)
                if not frozen[i] and points[i] == 0
            ]
            if missing:
    ...
    def standings_analytic(self, player_data: pd.DataFrame, n_outcomes: int = 6000, seed: int = 51):
        ...
        win_pct, exp_pts, swing = _an.locked_board_standings(pick_home, points, outcomes)
    ...
    def optimize_picks_hill_climb(
        ...
        if len(available_points) < len(games_to_pick):
            raise ValueError(
                f"Not enough confidence points ({len(available_points)}) for games ({len(games_to_pick)})"
            )
        ...
        for restart in range(restarts):
            ...
            for i in range(iterations):
                ...
                if (i + 1) % 100 == 0:
                    print(
                        f"   Iteration {i+1}/{iterations}: {current_prob:.4f} ({improvements} improvements)"
                    )

            print(
                f"   Final win probability: {current_prob:.4f} ({improvements} total improvements)"
            )
        ...
        for picks, win_prob in top_combinations:
            for team, points in picks.items():
                if team not in team_stats:
                    team_stats[team] = {"appearances": 0, "total_points": 0, "point_values": []}
        ...
        for _, row in summary_df.head(15).iterrows():
            ...
            print(
                f"   {team:<8} {freq:>6.1%} ({row['appearances']:>4})  {avg_conf:>5.1f}      {min_conf:.0f}-{max_conf:.0f} {indicator}"
            )
    ...
    def _generate_greedy_picks(
        ...
        sorted_games = sorted(
            games_to_pick, key=lambda g: abs(g.vegas_win_prob - 0.5), reverse=True
        )
    ...
    def _generate_random_picks(
        self, games_to_pick: List[Game], available_points: set
    ) -> Dict[str, int]:
    ...
    def _evaluate_picks(
        self, player_name: str, picks: Dict[str, int], all_fixed_picks: Dict[str, Dict[str, int]]
    ) -> float:
    ...
    def _get_neighbor_solution(
        self, current_picks: Dict[str, int], games_to_pick: List[Game], player_fixed: Dict[str, int]
    ) -> Dict[str, int]:
        ...
        if np.random.random() < 0.5:
            ...
            if game_to_swap and picked_team:
                ...
                other_team = (
                    game_to_swap.away_team
                    if picked_team == game_to_swap.home_team
                    else game_to_swap.home_team
                )
    ...
    def assess_remaining_game_importance(
        self, player_name: str, current_standings: dict, player_picks: dict
    ) -> pd.DataFrame:
        """
        ...
        your_current_rank = next(
            i for i, (name, _) in enumerate(sorted_standings, 1) if name == player_name
        )
        ...
        for game_idx, game in enumerate(remaining_games):
            ...
            for other_name, other_current_points in current_standings.items():
                ...
                if (
                    win_scenario_final > other_max_possible
                    and your_current_points <= other_current_points
                ):
                    players_you_could_pass += 1

                # If you lose this game, could they pass you?
                if (
                    other_max_possible > loss_scenario_final
                    and other_current_points <= your_current_points
                ):
                    players_who_could_pass_you += 1

            # Calculate importance based on Vegas probability and positional impact
            vegas_win_prob = (
                game.vegas_win_prob if your_pick == game.home_team else (1.0 - game.vegas_win_prob)
            )
            ...
            uncertainty_factor = (
                1.0 - abs(vegas_win_prob - 0.5) * 2
            )  # 0 to 1, higher for closer games
            confidence_factor = your_confidence / 16.0  # Normalize confidence
            position_factor = (players_you_could_pass + players_who_could_pass_you) / len(
                current_standings
            )
            scarcity_factor = (len(remaining_games) - game_idx) / len(
                remaining_games
            )  # Later games more important
