...
def _build_comparison_rows(comparison_df: Optional[pd.DataFrame]) -> List[Dict]:
...
def generate_html_report(
    *,
    ...
    comparison_rows = _build_comparison_rows(comparison_df)
...
"""
  ...
  <section class="panel">
    <div class="panel-head">
      ...
      <span class="panel-note">same field &amp; outcome draws per slate</span>
    </div>
    <div class="table-scroll">
...
(function(){{
  ...
  if(comparisonBody){{
    DATA.comparison.forEach(function(c){{
      ...
      var tdWin = document.createElement('td');
      tdWin.className = 'num';
      tdWin.textContent = (Math.round(c.win_probability*1000)/10) + '%';

      var tdStd = document.createElement('td');
      tdStd.className = 'num';
      tdStd.textContent = c.win_std.toFixed(4);
