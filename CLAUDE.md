...
- **`confidence_pickem_sim.py`** — the engine. `Game` and `Player` are dataclasses; `Player` carries three 0–1 behavioral knobs: `skill_level`, `crowd_following`, `confidence_following`. `ConfidencePickEmSimulator`:
  ...
  - `compare_slates(player_name, slates, ...)` — scores multiple full pick sets (`{label: {TEAM: confidence}}`) on the *same* analytical field/outcome draws, so differences reflect the slate choice, not simulation noise. Returns `win_probability` plus `win_std` and `downside_win_probability` (mean `P(win)` over the worst `downside_quantile` fraction of simulated weeks) so a boom/bust slate (one high-impact pick) can be told apart from a steadier one with similar win probability. Backs `--compare-slate` in `cli/optimize.py`, which also renders it as a panel in the `--html` report when both flags are combined.
